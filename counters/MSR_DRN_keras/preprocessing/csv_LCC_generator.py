...
class CSVLCCGenerator(object):
    ...
    def compute_keypoints_targets(self, image_shape, annotations_leaves_centers,):
        ...
        #assert len(np.where(annotations==1)[0])==annotations_leaves_centers.shape[0], 'there is some bug in the gaussian creation procses'
        '''
        # visualize the local-softmax
        kernal_size = (5,5)
        beta = 100
        paded_anno = np.pad(annotations, (((kernal_size[0]-1)//2,(kernal_size[0]-1)//2),((kernal_size[1]-1)//2,(kernal_size[1]-1)//2)), 'constant')
        ann_after_LSM = np.zeros(annotations.shape)
        for i in range(ann_after_LSM.shape[0]):
            for j in range(ann_after_LSM.shape[1]):
                ann_after_LSM[i,j] = np.max(paded_anno[i:i+kernal_size[0],j:j+kernal_size[1]])
        ann_after_LSM = np.abs(annotations-ann_after_LSM)
        ann_after_LSM = np.exp(-ann_after_LSM*beta)
        ann_after_LSM = annotations*ann_after_LSM
        seaborn.heatmap(ann_after_LSM)
    ...
    def random_transform_rbg_centers_images(self, rgb_image, annotations):
        ...
        if self.transform_generator:
            while(random_success_flag == False):
                ...
                # transform_centers = adjust_transform_for_image(transformation_to_apply , centers_image,
                #                                           self.transform_parameters.relative_translation)
                #
                # centers_transform_params = self.transform_parameters
                # centers_transform_params.interpolation = 'nearest'
                # res_centers_image = apply_transform(transform_centers, centers_image, centers_transform_params)
    ...
    def resize_image_320(self, image):
        return resize_image_320(image)#, min_side=self.image_min_side, max_side=self.image_max_side)
    ...
    def compute_targets(self, image_group, annotations_group_leaves_center, annotations_group_num_of_leaves):
        ...
        if self.option==1 or self.option == 9:
            ret = [regression_batch, np.expand_dims(labels_batch, 3)]
            # ret = [np.expand_dims(labels_batch, 3)]
        elif self.option == 3 or self.option == 10:
            ret = [regression_batch, np.expand_dims(labels_batch, 3), np.expand_dims(labels_batch, 3),np.expand_dims(labels_batch, 3),np.expand_dims(labels_batch, 3),np.expand_dims(labels_batch, 3)]#, np.expand_dims(labels_batch, 3)]
        elif self.option == 'reg_baseline_c5' or self.option == 'reg_fpn_p3' or self.option == 'reg_baseline_c5_dubreshko':
        ...
        elif self.option == 'reg_baseline_c5_dubreshko_nd':
            ret = [regression_batch, np.zeros(1)]
        return ret
    ...
    def compute_input_output(self, group):
        ...
        # print('image path - ' + self.image_path_rgb(group[0]))
        ...
        if self.csv_leaf_number_file:
            ...
            if self.option == 20 or self.option == 2:
