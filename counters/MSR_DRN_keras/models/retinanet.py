...
def default_regression_model(num_anchors, pyramid_feature_size=256, regression_feature_size=256, name='regression_submodel'):
    ...
    """
    # All new conv count_layers except the final one in the
...
def retinanet(
    ...
    Args
        inputs                  : keras.count_layers.Input (or list of) for the input to the model.
...
def retinanet_bbox(
    ...
    **kwargs
    ...
    This model uses the minimum retinanet model and appends a few count_layers to compute boxes within the graph.
    These count_layers include applying the regression values to the anchors and performing NMS.

    Args
        model             : RetinaNet model to append bbox count_layers to. If None, it will create a RetinaNet model using **kwargs.
